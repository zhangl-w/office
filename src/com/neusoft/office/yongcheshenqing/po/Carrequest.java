package com.neusoft.office.yongcheshenqing.po;

public class Carrequest extends Car{
	private int id;
	private String shenqingkeshi;
	private String qingqiushijian;
	private String shenqingren;

	private int shenqingcheshuliang;
	private String qingqiuliyou;
	private String chufashijian;
	private String jieshushijian;
	private String shenpiren;
	private String shenqingjieguo;
	private int shiyongtianshu;
	private String shifoushangbao;

	private String shenheyijian;
	private String shenheyijian2;
	private String shifouguidang;
	
	public String getShenheyijian2() {
		return shenheyijian2;
	}
	public void setShenheyijian2(String shenheyijian2) {
		this.shenheyijian2 = shenheyijian2;
	}
	public String getShenheyijian() {
		return shenheyijian;
	}
	public void setShenheyijian(String shenheyijian) {
		this.shenheyijian = shenheyijian;
	}
	public String getShifouguidang() {
		return shifouguidang;
	}
	public void setShifouguidang(String shifouguidang) {
		this.shifouguidang = shifouguidang;
	}
	public String getShifoushangbao() {
		return shifoushangbao;
	}
	public void setShifoushangbao(String shifoushangbao) {
		this.shifoushangbao = shifoushangbao;
	}
	
	public int getShiyongtianshu() {
		return shiyongtianshu;
	}
	public void setShiyongtianshu(int shiyongtianshu) {
		this.shiyongtianshu = shiyongtianshu;
	}
	public int getId() {
		return id;
	}
	public void setId(int id) {
		this.id = id;
	}
	public String getShenqingkeshi() {
		return shenqingkeshi;
	}
	public void setShenqingkeshi(String shenqingkeshi) {
		this.shenqingkeshi = shenqingkeshi;
	}
	
	public String getShenqingren() {
		return shenqingren;
	}
	public void setShenqingren(String shenqingren) {
		this.shenqingren = shenqingren;
	}
	
	public int getShenqingcheshuliang() {
		return shenqingcheshuliang;
	}
	public void setShenqingcheshuliang(int shenqingcheshuliang) {
		this.shenqingcheshuliang = shenqingcheshuliang;
	}
	public String getQingqiuliyou() {
		return qingqiuliyou;
	}
	public void setQingqiuliyou(String qingqiuliyou) {
		this.qingqiuliyou = qingqiuliyou;
	}
	
	public String getQingqiushijian() {
		return qingqiushijian;
	}
	public void setQingqiushijian(String qingqiushijian) {
		this.qingqiushijian = qingqiushijian;
	}
	public String getChufashijian() {
		return chufashijian;
	}
	public void setChufashijian(String chufashijian) {
		this.chufashijian = chufashijian;
	}
	public String getJieshushijian() {
		return jieshushijian;
	}
	public void setJieshushijian(String jieshushijian) {
		this.jieshushijian = jieshushijian;
	}
	public String getShenpiren() {
		return shenpiren;
	}
	public void setShenpiren(String shenpiren) {
		this.shenpiren = shenpiren;
	}
	public String getShenqingjieguo() {
		return shenqingjieguo;
	}
	public void setShenqingjieguo(String shenqingjieguo) {
		this.shenqingjieguo = shenqingjieguo;
	}
	
	
}
